...
### ¿Qué es eBPF y por qué aparece en esta guía?
eBPF (*extended Berkeley Packet Filter*) es una tecnología del kernel de Linux que permite ejecutar pequeños programas dentro del kernel mismo, de forma segura, para observar eventos del sistema (llamadas al sistema, acceso a archivos, tráfico de red) sin necesidad de escribir un módulo de kernel tradicional. El módulo FIM/whodata de Wazuh 4.12 lo usa para detectar en tiempo real qué proceso modificó un archivo.

Estos programas eBPF se compilan con Clang usando una técnica llamada **CO-RE** (*Compile Once – Run Everywhere*): el binario resultante puede ejecutarse en kernels con estructuras internas ligeramente distintas a la del kernel donde se compiló, gracias a información de depuración (BTF) que el kernel lee en tiempo de carga. Para esto, Clang necesita macros y estructuras específicas del kernel (`__builtin_preserve_access_index`, tipos de mapas BPF, etc.) que en un sistema `glibc` normalmente llegan con el paquete `linux-headers`, pero que en Alpine vienen incompletos — de ahí las cabeceras de compatibilidad que se crean en el Paso 2.

### ¿Qué son los sub-builds de CMake dentro del Makefile de Wazuh?
No todo el código de Wazuh se compila con `make`. Dos componentes — `sysinfo`/`data_provider` (recolección de información del sistema) y `syscollector` (inventario de hardware/software) — son proyectos independientes que usan **CMake**, otra herramienta de generación de builds, más común en proyectos C++. El Makefile principal de Wazuh, al llegar a esos componentes, invoca a CMake como un proceso hijo para que los compile.

Esto importa para la guía porque CMake **no hereda automáticamente** las variables de entorno de compilación (como `LDFLAGS`) de la forma en que lo hace `make`; cualquier flag de enlace que dependa la compatibilidad con `musl` hay que pasárselo explícitamente vía la variable `CMAKE_OPTS`, como se hace en el Paso 7.
...
Una vez en este punto, siempre que se quiera volver a compilar el agente se reducirá el tiempo de espera, porque `make` solo recompila lo que cambió: las dependencias de `src/external` (OpenSSL, `libbpf-bootstrap`, etc.) y los sub-builds de CMake ya quedaron resueltos y no se vuelven a construir desde cero. Si solo se modifica un archivo fuente propio de Wazuh, basta con repetir el comando de este paso — no hace falta repetir los Pasos 3 a 6.
...
> puerto de **conexión persistente** (Paso 10, una vez arrancado el agente)
...
> **Nota:** si siguieron el Paso 3.1 tal como está escrito (dejando que
> `make deps` traiga la versión **precompilada** de `libbpf-bootstrap`), este
> caso de error **no debería aplicarles** — la carpeta `build/bpftool/`
> simplemente no se genera con el binario precompilado, así que un `find
> -name "bpf.h"` no encontrará nada, y eso es lo esperado, no una falla. Este
> caso queda documentado solo para quien fuerce la compilación de
> `libbpf-bootstrap` **desde fuente** en vez de usar el precompilado.

Al compilar `libbpf-bootstrap` **desde fuente** puede fallar con:

```
fatal error: bpf/bpf.h: No such file or directory
```

Es un bug documentado del propio `bpftool` (parche del kernel de nov. 2021):
el Makefile de la versión "bootstrap" no instala las cabeceras de libbpf en
la ruta que su propio `main.c` espera, aunque sí las genera en otro lugar del
árbol de build. La corrección, solo para ese escenario:

```bash
find /root/wazuh/src/external/libbpf-bootstrap -name "bpf.h"
cp /root/wazuh/src/external/libbpf-bootstrap/build/bpftool/bootstrap/libbpf/include/bpf/*.h /usr/include/bpf/
```

### Si la IP y el puerto no son correctos
Si durante la instalación se agregó mal la IP o el puerto del manager, o el agente quedó configurado con otro protocolo de conexión, estos valores se pueden corregir manualmente en el archivo de configuración del agente, sin necesidad de reinstalar nada:

Editar `/var/ossec/etc/ossec.conf`, dentro de la sección `<client>`:
...
3. **Obtener la llave manualmente vía la API de Wazuh**, si `agent-auth` sigue fallando pero el manager sí responde. Esto es útil porque separa el problema de red del problema de generación de la llave: si la API logra entregar una llave, el problema estaba en `wazuh-authd`/puerto 1515, no en el manager en general.
   ...
   # 2. Registrar el agente por API (si aún no existe) y obtener su id
   curl -k -X POST "https://IP_DEL_MANAGER:55000/agents" \
     -H "Authorization: Bearer $TOKEN" \
     -H "Content-Type: application/json" \
     -d '{"name": "nombre-del-agente"}'

   # 3. Con el id devuelto en el paso anterior, pedir la llave en texto plano
   curl -k -X GET "https://IP_DEL_MANAGER:55000/agents/<AGENT_ID>/key?raw=true" \
     -H "Authorization: Bearer $TOKEN"
   ```

   La llave que devuelve el último comando se importa **en el agente**, no en el manager, con:
   ```bash
...
los pasos de esta guía corresponden a errores reales encontrados y
resueltos durante una sesión de depuración completa — no es teoría, es la
ruta que efectivamente funcionó.
