# Guía: Compilar el agente de Wazuh 4.12.0 en Alpine Linux 3.21.5 (musl)

> Wazuh no da soporte oficial a Alpine/musl. Esta guía documenta, paso a paso,
> todos los parches necesarios para lograr una compilación limpia, encontrados
> mediante depuración iterativa real (no es la ruta "oficial" de Wazuh, que
> asume glibc).
...
## Paso 5 — [NO APLICA si se siguió el Paso 3.1] Cabeceras faltantes de `bpftool` bootstrap

> **Si ya aplicaron el Paso 3.1 (versión precompilada de `libbpf-bootstrap`),
> salten directo al Paso 6.** Este paso solo existía para el camino viejo
> (compilar `libbpf-bootstrap` desde fuente) y con la versión precompilada
...
Al compilar `libbpf-bootstrap` **desde fuente** (no el camino recomendado de
esta guía), puede fallar con:
...
## Paso 6 — Compatibilidad de símbolos glibc faltantes en musl
...
### 6.1 — Compilar `libexecinfo` (para `backtrace`/`backtrace_symbols`)
...
### 6.2 — Compilar `libucontext` (para `getcontext`/`setcontext`/`makecontext`)
...
### 6.3 — El shim de compatibilidad completo (`musl_compat.c`)
...
## Paso 7 — Inyectar el shim en el Makefile de Wazuh
...
## Paso 8 — Compilación completa del agente (con `CMAKE_OPTS` para los sub-builds de CMake)
...
**Resultado del Paso 8, confirmado en vivo:** el build completa con
`Done building agent` — de punta a punta, sin errores adicionales más allá
de los ya documentados.

---

## Paso 9 — Instalar el agente compilado
...
## Paso 10 — Configurar `ossec.conf` y registrar el agente
...
## Paso 11 — Arrancar el agente (parche de `wazuh-control` para BusyBox)
