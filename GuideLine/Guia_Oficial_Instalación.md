...
> Si hay error en la compilación de `libbpf-bootstrap` desde fuente y con la versión precompilada
> ni siquiera se genera la carpeta `build/bpftool/` — por eso un `find
> -name "bpf.h"` no encuentra nada: no es un error, es que este paso ya no
> aplica. Se deja documentado por si en algún momento se fuerza la
> compilación desde fuente en vez de usar el precompilado.
...
Si durante la instalación se agrego mal la IP, el puerto o simplemente el agente asumio otro protocolo de conexión, estos se pueden cambiar en la siguiente ruta:
...
## Fallo al obtener la llave de wazuh manager

Si al ejecutar el paso 9 no se logra registrar el agente hay varias posibilidades:
- Revisar que el servidor web tenga conexión con la IP wazuh manager en el puerto ***
- Ya existe un agente con ese nombre (pudo haberse generado por intentos fallidos), para este caso se puede entrar al dashboard y eliminar el agente o cambiar en nombre y registrar un nuevo agente bajo otro nombre con el siguiente comando:
-   (bash de auth IP_manager pero con nombre).
-   Si el problema es la generación de la llave por parte de wazuh, esta se puede obtener a través de los siguientes pasos:
-     Ingresando a wazuh dashboard, en el menu lateral -> server manager -> Dev Tools. Esto nos llevará a la siguiente interfaz con la que podremos interactuar con la API de wazuh directamente:
- <img width="1919" height="704" alt="image" src="https://github.com/user-attachments/assets/396121f1-f3e7-428e-bf2a-388248b8b37a" />
Mediante el siguiente comando se puede obtener la llave para el id del agente y poder conectarlo, queda a disposición de los estudiantes averiguar los comandos para llevar esa llave al servidor web y registrarse ante el manager.
